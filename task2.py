...
# - Uses best BPE configuration from Task 1 (k=2000, aggressive normalization)
...
# - Saves results to task2_results.json
...
# Configuration
PERCENTAGE = 0.10
BEST_NORMALIZATION = "lower_nopunct"  # Best from Task 1 results
TOP_MERGE_COUNTS = [1000, 2000]  # Top 2 merge counts from Task 1
...
class NGramLanguageModel:
    ...
    def tune_interpolation_weights(self, token_stream, bos_token='<s>'):
        ...
        for step in INTERPOLATION_STEPS:
            weights = np.array([step, (1 - step) / (self.n_order - 1)] * (self.n_order - 1) + [step])
            weights = weights[:self.n_order]
...
def main():
    ...
    for merge_count in TOP_MERGE_COUNTS:
        ...
        for n in N_GRAM_ORDERS:
            ...
            ngram_model = NGramLanguageModel(n, LAPLACE_ALPHA)
